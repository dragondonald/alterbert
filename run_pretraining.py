...
"""
python3 run_pretraining.py --bert_config_file /data/lxz/alterbert/data/bert_config.json --input_file /data/lxz/nlp_data/tfrecord/128/THUCNews/train.txt --output_dir /data/lxz/alterbert/model/test/ --do_train True --train_batch_size 128 --num_train_steps 100
"""

## Required parameters
flags.DEFINE_string(
    "bert_config_file", "/data/lxz/alterbert/data/bert_config.json",
    "The config json file corresponding to the pre-trained BERT model. "
    "This specifies the model architecture.")

flags.DEFINE_string(
    "input_file", "/data/lxz/nlp_data/tfrecord/128/THUCNews/train.txt",
    "Input TF example files (can be a glob or comma separated).")

flags.DEFINE_string(
    "output_dir", "/data/lxz/alterbert/model/test/",
...
flags.DEFINE_integer("keep_checkpoint_max", 2,
                     "keep_checkpoint_max.")
...
flags.DEFINE_bool(
    "use_random_next", False,
    "Whether to use random next sentence concat rather than whole original document.")
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
                     ...
                     use_one_hot_embeddings, use_random_next):
...
def _decode_record(record, name_to_features):
  ...
  ## tf.Example only supports tf.int64, but the TPU only supports tf.int64.
  ## So cast all int64 to int64.
...
def normal_estimator(model_fn, model_dir, params):
    keep_checkpoint_max = params.get('keep_checkpoint_max', 3)
    save_checkpoints_steps = params.get('save_checkpoints_steps', 10000)
    log_step_count_steps = params.get('log_step_count_steps', 100)
    gpu_allow_growth = params.get('gpu_allow_growth', False)
    ...
    estimator = tf.estimator.Estimator(model_fn, model_dir=model_dir, config=run_config)
    return estimator


def tpu_estimator(model_fn, model_dir, params):
    keep_checkpoint_max = params.get('keep_checkpoint_max', 3)
    save_checkpoints_steps = params.get('save_checkpoints_steps', 10000)
    log_step_count_steps = params.get('log_step_count_steps', 100)
    use_tpu = params.get('use_tpu', False)
    tpu_name = params.get('tpu_name', None)
    tpu_zone = params.get('tpu_zone', None)
    gcp_project = params.get('gcp_project', None)
    master = params.get('master', None)
    iterations_per_loop = params.get('iterations_per_loop', 1000)
    num_tpu_cores = params.get('num_tpu_cores', 8)
    train_batch_size = params.get('train_batch_size', 32)
    eval_batch_size = params.get('eval_batch_size', 8)
...
def get_estimator(use_tpu, model_fn, model_dir, **kwargs):
    kwargs['use_tpu'] = use_tpu
    return tpu_estimator(model_fn, model_dir, kwargs)
...
def main(_):
    ...
    bert_config = config.BertConfig.from_json_file(FLAGS.bert_config_file)
    ...
    model_fn = model_fn_builder(
        ...
        use_random_next=FLAGS.use_random_next)
    estimator = get_estimator(use_tpu=FLAGS.use_tpu, model_fn=model_fn, model_dir=FLAGS.output_dir,
                              keep_checkpoint_max=FLAGS.keep_checkpoint_max,
                              save_checkpoints_steps=FLAGS.save_checkpoints_steps,
                              log_step_count_steps=FLAGS.log_step_count_steps,
                              gpu_allow_growth=FLAGS.gpu_allow_growth,
    ...
    if FLAGS.do_train:
        ...
        hooks = []
        # profiler_hook = tf.train.ProfilerHook(output_dir=FLAGS.output_dir, save_steps=10)
